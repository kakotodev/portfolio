    ...
    return(
        <>
            {Object.entries(skillsPerCategory). map(([nameCategory, listSkills]) =>(
                <div key={nameCategory} className="">
                    <h2>{nameCategory}</h2>
                    <div className="flex justify-center border">
                        <div className="flex justify-center flex-wrap gap-5 h-64 w-[300px] mx-10 my-8">
                            {listSkills.map((skill) => (
                                skill.urlImg ? (
                                    <div key={skill.id} className="flex relative group justify-center hover:grayscale-0 mx-5 my-3 grayscale transition-all">
